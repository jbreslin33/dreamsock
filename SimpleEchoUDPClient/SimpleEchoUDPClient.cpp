...
int servLen = sizeof(inetServAddr);
...
void ClientProcess(void)
{
	...
	while(connectionOpen)
	{
		...
		if(gets(transmitBuf))
		{
			...
			if(sendto(Socket, transmitBuf, 2, 0,
				(struct sockaddr *) &inetServAddr, servLen) == SOCKET_ERROR)
			{
				connectionOpen = 0;
			}
		}

		// If the letter the user entered is "q", stop the application.
		...
		if(recvfrom(Socket, receiveBuf, 2, 0, NULL, NULL))
		{
