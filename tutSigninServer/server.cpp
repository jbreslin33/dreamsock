...
void CArmyWarServer::MovePlayer(clientData *client)
{
	...
	{
		client->command.bullet.shot = true;

		if(client->command.heading == NORTH)
		{
			client->command.bullet.vel.x = 0.0f;
			client->command.bullet.vel.y = -200.0f;
		}
		if(client->command.heading == SOUTH)
		{
			client->command.bullet.vel.x = 0.0f;
			client->command.bullet.vel.y = 200.0f;
		}
		if(client->command.heading == EAST)
		{
			client->command.bullet.vel.x = 200.0f;
			client->command.bullet.vel.y = 0.0f;
		}
		if(client->command.heading == WEST)
		{
			client->command.bullet.vel.x = -200.0f;
			client->command.bullet.vel.y = 0.0f;
		}

		if(client->command.heading == NORTHEAST)
		{
			client->command.bullet.vel.x = 200.0f;
			client->command.bullet.vel.y = -200.0f;
		}
		if(client->command.heading == NORTHWEST)
		{
			client->command.bullet.vel.x = -200.0f;
			client->command.bullet.vel.y = -200.0f;
		}
		if(client->command.heading == SOUTHEAST)
		{
			client->command.bullet.vel.x = 200.0f;
			client->command.bullet.vel.y = 200.0f;
		}
		if(client->command.heading == SOUTHWEST)
		{
			client->command.bullet.vel.x = -200.0f;
			client->command.bullet.vel.y = 200.0f;
		}
