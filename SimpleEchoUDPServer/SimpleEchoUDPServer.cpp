...
int InitSockets(void)
{
	...
	error = bind(Socket, servAddr, sizeof(sockaddr));
...
void ServerProcess(void)
{
	...
	clientLen = sizeof(inetClientAddr);
	...
	while(connectionOpen)
	{
		...
		if(recvfrom(Socket, buf, 2, 0, (struct sockaddr *) &inetClientAddr, &clientLen))
		{
			...
			// Send the feedback.
			if(sendto(Socket, buf, 2, 0,
				(struct sockaddr *) &inetClientAddr, clientLen) == SOCKET_ERROR)
			{
				connectionOpen = 0;
			}
		}
