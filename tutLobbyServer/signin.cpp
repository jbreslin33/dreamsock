...
/*
				...
				try 
				{
					...
					query << "INSERT INTO playerdata VALUES (NULL,\"" << nickname << 
						"\",\"" << firstname <<
						"\",\"" << surname <<
						"\"," << age <<
						",\"" << gender <<
					...
					// -> Send account created successfully message back to client...
					respond = SIGNIN_RESULT_ACCEPTED;

				}
				catch (BadQuery er) // handle any connection errors
				{ 
					// -> Send a mySQL Error back to the client (this should never happen)
					respond = SIGNIN_RESULT_MYSQLERROR;
				}
*/
				// TEMP FIXME!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				respond = SIGNIN_RESULT_MYSQLERROR;
